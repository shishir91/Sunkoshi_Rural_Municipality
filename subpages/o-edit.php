...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$hasError) {
        ...
        $sql = "UPDATE organization
            SET otype=?, orgname=?, founame=?, doe=?, phone=?, email=?, ward=?, tole=?
            WHERE o_id = ?";

        if ($stmt = $conn->prepare($sql)) {
            //bind variables
            $stmt->bind_param("sssssssi", $o_type, $o_oname, $o_fname, $o_doe, $o_phone, $o_email, $o_ward, $o_tole, $o_id);
    ...
    $sql = "SELECT otype, orgname, founame, doe, phone, email, ward, tole
         FROM organization WHERE o_id=$id";
