...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$hasError) {
        ...
        $sql = "UPDATE personal
            SET firstname = ?, midname = ?, lastname = ?, gender = ?,
            dob = ?, ward = ?, tole = ?, phone = ?, email = ?, prof = ?
            WHERE id = ?";

        if ($stmt = $conn->prepare($sql)) {
            //bind variables
            $stmt->bind_param("ssssssssssi", $p_fname, $p_mname, $p_lname, $p_gen, $p_dob, $p_ward, $p_tole, $p_phone, $p_email, $p_prof, $p_id);
    ...
    $sql = "SELECT firstname, midname, lastname, gender, dob, ward, tole, phone, email, prof 
            FROM personal WHERE id=$id";
...
<div class="container">
    ...
    <div class="row">
        <div class="col-lg-6 col-md-9 col-sm-12">
            <form method="POST" action="dashindex.php?page=p-edit">
